...
Відправити GET-запит на будь-який ендпоінт з токеном. Якщо отримаєте відповідь з `success: true` — з'єднання працює.
...
- **API токен** — запросити у адміністратора сайту (сторінка `/admin/api-keys`)
- **Проблеми з API** — всі запити логуються на сайті в `/admin/api-keys` → вкладка "Лог запитів"

---

*Документ сформовано автоматично на основі реалізованого API. Всі ендпоінти, формати запитів та відповідей — фінальні.*
