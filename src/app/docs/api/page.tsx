...
// ================================================================
//  Public API Documentation — read-only, no auth required
//  URL: /docs/api (noindex, not visible to search engines)
// ================================================================

import { useState, useCallback } from "react";
import {
  Package, Users, ShoppingBag, FileText, CreditCard, DollarSign,
  Heart, Gift, ChevronRight, Copy, Check, ArrowRight,
  Code2, Shield, Zap, AlertTriangle, Globe, BookOpen,
} from "lucide-react";
...
// ─── Code generators ───
function curlExample(e: DocEndpoint): string {
...
function jsExample(e: DocEndpoint): string {
  const url = `${BASE_URL}${e.path}`;
...
function pythonExample(e: DocEndpoint): string {
  const url = `${BASE_URL}${e.path}`;
...
function EndpointDetail({ endpoint }: { endpoint: DocEndpoint }) {
  const [codeLang, setCodeLang] = useState<"curl" | "js" | "python" | "1c">("1c");

  const codeFn = { curl: curlExample, js: jsExample, python: pythonExample, "1c": onecExample };
  const code = codeFn[codeLang](endpoint);
    ...
    <div className="space-y-5">
      ...
      <div>
        ...
        <div className="flex gap-1 mb-2">
          {(["1c", "curl", "js", "python"] as const).map(lang => (
            <button
              key={lang}
      ...
      <aside className={`${sidebarOpen ? "w-64" : "w-0"} shrink-0 overflow-y-auto border-r border-[#1e1e2a] bg-[#0a0a0f] transition-all`}>
        <div className="p-4">
          ...
          <nav className="space-y-3">
            ...
            {API_DOCS.map(group => (
              <SidebarGroup key={group.id} group={group} selectedId={selectedEndpoint.id} onSelect={setSelectedEndpoint} />
            ))}
          </nav>
        </div>
      </aside>
        ...
        <div className="mx-auto max-w-4xl px-6 py-8 space-y-10">
          ...
          <div className="border-t border-[#1e1e2a] pt-8">
            ...
            <div className="space-y-3 text-sm text-zinc-400">
              ...
              <p>Токен отримуєте від адміністратора сайту.</p>
          ...
          <div className="border-t border-[#1e1e2a] pt-8">
            ...
            <div className="space-y-3 text-sm text-zinc-400">
              <p>GET-запити підтримують: <code className="rounded bg-[#1a1a24] px-1.5 py-0.5 text-xs text-zinc-300">?page=1&per_page=50</code></p>
              <p>Максимум: 500 записів. Відповідь містить <code className="rounded bg-[#1a1a24] px-1.5 py-0.5 text-xs text-zinc-300">meta</code> з total, page, per_page, total_pages.</p>
            </div>
          </div>
          ...
          <div className="border-t border-[#1e1e2a] pt-8 pb-16">
            ...
            <div className="space-y-3 text-sm text-zinc-400">
              <p>Ліміт: 100 запитів на хвилину (за замовчуванням).</p>
              <p>Заголовки: <code className="rounded bg-[#1a1a24] px-1.5 py-0.5 text-xs text-zinc-300">X-RateLimit-Limit</code>, <code className="rounded bg-[#1a1a24] px-1.5 py-0.5 text-xs text-zinc-300">X-RateLimit-Remaining</code></p>
              <p>При перевищенні: HTTP 429 + <code className="rounded bg-[#1a1a24] px-1.5 py-0.5 text-xs text-zinc-300">Retry-After</code>.</p>
            </div>
          </div>
        </div>
      </main>
