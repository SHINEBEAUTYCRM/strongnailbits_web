    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="flex flex-col gap-2">
          {stats.columnCounts.map((col) => (
            <div key={col.id} className="flex items-center gap-3">
              ...
              <span className="text-xs flex-1" style={{ color: "#d4d4d8" }}>{col.label}</span>
              <span
        ...
        <div className="flex flex-col gap-2">
          {stats.priorityCounts.map((p) => (
            <div key={p.id} className="flex items-center gap-3">
              ...
              <span className="text-xs flex-1" style={{ color: "#d4d4d8" }}>{p.label}</span>
              <span
      ...
      <DashCard title="Навантаження команди">
        {stats.memberStats.length === 0 ? (
          <p className="text-xs" style={{ color: "#52525b" }}>Немає учасників</p>
        ) : (
      ...
      <DashCard title="Прострочені" badge={stats.overdue.length > 0 ? stats.overdue.length : undefined}>
        {stats.overdue.length === 0 ? (
          <p className="text-xs" style={{ color: "#52525b" }}>Все вчасно 🎉</p>
        ) : (
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-3">
        <h3 className="text-xs font-semibold" style={{ color: "#a1a1aa" }}>
          {title}
        </h3>
        {badge !== undefined && (
          <span
