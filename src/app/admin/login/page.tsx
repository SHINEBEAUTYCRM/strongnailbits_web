  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!profile || !["admin", "manager"].includes(profile.role || "")) {
        setError("У вас немає прав доступу до адмін-панелі");
      ...
      if (!profile.admin_approved) {
        setError("Ваш акаунт очікує підтвердження адміністратором");
        await supabase.auth.signOut();
        setLoading(false);
        return;
      }
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center" style={{ background: "#08080c" }}>
      <div className="w-full max-w-[400px] mx-4">
        ...
        <div className="flex items-center justify-between mt-4 px-1">
          <Link href="/admin/reset-password" className="text-xs" style={{ color: "#52525b" }}>
            Забули пароль?
          </Link>
          <Link href="/admin/register" className="inline-flex items-center gap-1.5 text-xs" style={{ color: "#52525b" }}>
            <UserPlus className="w-3.5 h-3.5" /> Запросити доступ
          </Link>
        </div>
