...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const categoryMap = new Map((existingCategories ?? []).map((c) => [c.name_uk.toLowerCase(), c.id]));
    ...
    for (let i = structure.data_start_row; i < rows.length; i++) {
      ...
      if (productData.brand_name) {
        ...
        brandId = brandMap.get(brandLower) ?? null;
        if (!brandId) {
          // Create new brand
      ...
      if (productData.category_path) {
        ...
        categoryId = categoryMap.get(catLower) ?? null;
      }
      ...
      if (existingProduct) {
        ...
        if (import_mode === "update" || import_mode === "create_or_update" || duplicate_strategy === "update") {
          ...
          if (upsertData.price && oldPrice !== newPrice) {
            priceChanges.push({
              name: String(productData.name_uk),
              old_price: oldPrice,
              new_price: newPrice,
            });
          }
      ...
      const { error } = await supabase.from("products").insert(upsertData);
      if (error) {
        errors.push({ row: i, error: error.message });
        skipped++;
      } else {
    ...
    const report: PostImportReport = {
      ...
      price_changes: {
        average_change_percent: priceChanges.length > 0
          ? Math.round(
              (priceChanges.reduce((sum, c) => sum + ((c.new_price - c.old_price) / c.old_price) * 100, 0) /
    ...
    if (errors.length > 0) {
      report.recommendations.push({
        type: "warning",
        message: `${errors.length} рядків з помилками при імпорті`,
      });
    }
    if (newBrands.length > 0) {
      report.recommendations.push({
        type: "info",
        message: `Створено нові бренди: ${newBrands.join(", ")}`,
      });
    }
    if ((noPhotoCount ?? 0) > 0) {
      report.recommendations.push({
        type: "warning",
        message: `${noPhotoCount} товарів без фото`,
      });
    }
    if ((noDescCount ?? 0) > 0) {
      report.recommendations.push({
        type: "info",
        message: `${noDescCount} товарів без опису`,
      });
    }
