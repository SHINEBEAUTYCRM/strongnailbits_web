...
export async function syncProducts(): Promise<SyncResult> {
  ...
  try {
    ...
    /* ---- 5. Safe sync: protect descriptions & images ---- */
    /*  New products → insert all fields                      */
    /*  Existing products → update ONLY safe fields:          */
    /*    price, old_price, quantity, status, category_id,    */
    /*    sku, weight, meta_title, meta_description,          */
    /*    cs_cart_updated_at, cs_cart_url, properties, slug    */
    /*    name_uk/name_ru/description_uk/description_ru →     */
    /*      only if currently empty in DB                     */
    /*    main_image_url/images → only if currently empty     */

    console.info("[sync:products] Loading existing products for safe merge...");
    ...
    while (exHasMore) {
      const { data: exProducts, error: exError } = await supabase
        .from("products")
        .select("id, cs_cart_id, name_uk, name_ru, description_uk, description_ru, main_image_url, images")
        .range(exOffset, exOffset + exPageSize - 1);

      if (exError || !exProducts || exProducts.length === 0) {
        exHasMore = false;
      } else {
        for (const p of exProducts) {
          if (p.cs_cart_id) {
            existingMap.set(p.cs_cart_id as number, {
              id: p.id as string,
              name_uk: p.name_uk as string | null,
              name_ru: p.name_ru as string | null,
              description_uk: p.description_uk as string | null,
              description_ru: p.description_ru as string | null,
        ...
        exOffset += exPageSize;
        exHasMore = exProducts.length === exPageSize;
      }
    }

    console.info(`[sync:products] Existing products loaded: ${existingMap.size}`);

    // Split into new products (insert) and existing (update)
    const newRows: ProductRow[] = [];
    const updateRows: ProductRow[] = [];
    ...
    for (const row of rows) {
      ...
      if (!existing) {
        ...
        // Protect fields that were populated via import
        const safeRow = { ...row };

        // Names: only fill if empty in DB
        if (existing.name_uk && existing.name_uk.length > 0) {
          safeRow.name_uk = existing.name_uk;
        }
        if (existing.name_ru && existing.name_ru.length > 0) {
          safeRow.name_ru = existing.name_ru;
        }

        // Descriptions: NEVER overwrite if already populated
        if (existing.description_uk && existing.description_uk.length > 10) {
          safeRow.description_uk = existing.description_uk;
        }
        if (existing.description_ru && existing.description_ru.length > 10) {
          safeRow.description_ru = existing.description_ru;
        }

        // Images: NEVER overwrite if already populated
        if (existing.main_image_url && existing.main_image_url.length > 0) {
          safeRow.main_image_url = existing.main_image_url;
        }
        if (existing.images && existing.images.length > 0) {
          safeRow.images = existing.images;
        }

        updateRows.push(safeRow);
      }
    }

    console.info(
      `[sync:products] New: ${newRows.length}, Update: ${updateRows.length}`,
    );
