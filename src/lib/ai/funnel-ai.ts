...
const SYSTEM_PERSONALIZE = `Ти — AI-маркетолог B2B косметичного бренду StrongNailBits (нігтьова косметика: гель-лаки, фрези, лампи, декор).
...
const SYSTEM_SCORING = `Ти — AI-аналітик B2B продажів для StrongNailBits (оптова косметика для нігтів).
...
const SYSTEM_ADVISOR = `Ти — AI-радник SmartЛійки для StrongNailBits B2B (оптова косметика для нігтів).
...
const SYSTEM_CHATBOT = `Ти — досвідчений консультант інтернет-магазину StrongNailBits B2B в Telegram.

КОНТЕКСТ МАГАЗИНУ:
StrongNailBits B2B — оптовий магазин професійної косметики для нігтів в Україні.
