  ...
  return (
    ...
    >
      ...
      <div className="p-3 border-b" style={{ borderColor: 'rgba(255, 255, 255, 0.06)' }}>
        ...
        >
          <Search className="w-4 h-4 flex-shrink-0" style={{ color: '#4b5563' }} />
          <input
            type="text"
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            placeholder="Назва, SKU, бренд..."
            className="flex-1 bg-transparent text-sm outline-none placeholder:text-[#4b5563]"
            style={{ color: '#e5e7eb' }}
          />
          {searchQuery && (
          ...
          {isSearching && <Loader2 className="w-3.5 h-3.5 animate-spin" style={{ color: '#a855f7' }} />}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-1">
        {searchResults.length === 0 && searchQuery && !isSearching && (
          <div className="text-center py-8">
            <Package className="w-8 h-8 mx-auto mb-2" style={{ color: '#1e1e2a' }} />
        ...
        {searchResults.length === 0 && !searchQuery && (
          <div className="text-center py-8">
            <Search className="w-8 h-8 mx-auto mb-2" style={{ color: '#1e1e2a' }} />
            <p className="text-xs" style={{ color: '#4b5563' }}>
        ...
        {searchResults.map((product) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs truncate" style={{ color: '#e5e7eb' }}>
                {product.name}
              </p>
              <p
