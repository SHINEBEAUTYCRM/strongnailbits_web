...
const EMPTY: BrandData = {
  ...
  source_urls: [], source_notes: "", ai_prompt_context: "",
};
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="rounded-xl p-5" style={{ background: "var(--a-bg-card)", border: "1px solid var(--a-border)" }}>
            <h3 className="text-sm font-medium mb-4 flex items-center gap-2" style={{ color: "var(--a-text-2)" }}>
              <span className="text-base">🤖</span> AI Джерела інформації
            </h3>
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-medium mb-1.5" style={{ color: "var(--a-text-3)" }}>
                  URL сайтів (для парсингу описів)
                </label>
                <div className="space-y-2">
                  {form.source_urls.map((url, i) => (
                    <div key={i} className="flex gap-2">
                      <input
                        ...
                        value={url}
                        onChange={(e) => {
                          const updated = [...form.source_urls];
                          updated[i] = e.target.value;
                          setForm((f) => ({ ...f, source_urls: updated }));
                        }}
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          setForm((f) => ({ ...f, source_urls: f.source_urls.filter((_, j) => j !== i) }));
                        }}
                        className="px-2.5 py-2 rounded-lg text-sm"
                        style={{ color: "#f87171", border: "1px solid var(--a-border)" }}
                      >
                        ✕
                      </button>
                    </div>
                  ...
                  <button
                    type="button"
                    onClick={() => setForm((f) => ({ ...f, source_urls: [...f.source_urls, ""] }))}
              ...
              <TextArea
                label="Нотатки про бренд (AI буде використовувати)"
                value={form.source_notes}
                onChange={(v) => set("source_notes", v)}
                rows={4}
              />

              <TextArea
                label="Додатковий промпт-контекст"
                value={form.ai_prompt_context}
                onChange={(v) => set("ai_prompt_context", v)}
                rows={3}
              />
              <p className="text-[11px]" style={{ color: "var(--a-text-5)" }}>
                Специфічні інструкції для AI при генерації описів товарів цього бренду
              </p>
            </div>
          </div>
        </div>
